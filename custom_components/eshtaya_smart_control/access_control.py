"""Role-based access control and permission metadata for Eshtaya Smart Control."""
...
BUILTIN_ROLES: dict[str, dict[str, Any]] = {
    ...
    "viewer": {
        "name": "Viewer",
        "permissions": [
            "dashboard.view", "entity.view", "tuya.view", "multi.view",
            "docs.view", "system.view",
        ],
    },
    "operator": {
        "name": "Operator",
        "permissions": [
            "dashboard.view", "entity.view", "tuya.view", "tuya.control",
            "multi.view", "multi.control", "docs.view", "system.view",
        ],
    },
    "technician": {
        "name": "Technician",
        "permissions": [
            "dashboard.view", "entity.view", "entity.manage", "tuya.view",
            "tuya.control", "multi.view", "multi.control", "multi.manage",
            "docs.view", "system.view", "system.actions", "system.reports",
        ],
    },
...
class AccessControlManager:
    """Persist integration roles plus safe metadata for native HA access changes."""
    ...
    async def async_load(self) -> None:
        ...
        if isinstance(loaded, dict):
            ...
            data["ha_access"] = dict(loaded.get("ha_access") or {})
    ...
    def _roles(self) -> dict[str, dict[str, Any]]:
        ...
        for role_id, role in self._data.get("roles", {}).items():
            roles[role_id] = {
                ...
                "permissions": sorted(
                    set(role.get("permissions") or []) & set(PERMISSIONS)
                ),
                "custom": True,
            }
        return roles
    ...
    def permissions_for(self, user) -> set[str]:
        ...
        role_id = str(
            assignment.get("role")
            or self._data["settings"].get("default_role")
            or "no_access"
        )
    ...
    def public_access(self, user) -> dict[str, Any]:
        ...
        return {
            ...
            "is_owner": bool(user and getattr(user, "is_owner", False)),
            "permissions": sorted(perms),
            "modules": {
                module: perm in perms for module, perm in MODULE_PERMISSION.items()
            },
        }
            ...
            users.append(
                {
                    ...
                    "system_generated": getattr(user, "system_generated", False),
    ...
    async def async_assign_user(
        ...
        if target.is_admin:
            raise ValueError(
                "Administrator integration access is always full and cannot be restricted"
            )
        ...
        self.audit(
            actor,
            "integration_permissions_updated",
            user_id,
            {
                "role": role,
                "allow": clean_allow,
    ...
    async def async_save_role(
        self, actor, role_id: str, name: str, permissions: list[str]
    ) -> None:
        role_id = re.sub(r"[^a-z0-9_]+", "_", role_id.strip().lower()).strip("_")
        if not role_id or role_id in BUILTIN_ROLES:
            raise ValueError("Use a unique custom role id")
        self._data["roles"][role_id] = {
            "name": name.strip() or role_id,
            "permissions": sorted(set(permissions) & set(PERMISSIONS)),
        }
        self.audit(
            actor,
            "integration_role_saved",
            role_id,
            {"permissions": self._data["roles"][role_id]["permissions"]},
        )
        await self._store.async_save(self._data)
    ...
    async def async_delete_role(self, actor, role_id: str) -> None:
        ...
        for assignment in self._data["users"].values():
            if assignment.get("role") == role_id:
                assignment["role"] = self._data["settings"].get(
                    "default_role", "no_access"
                )
        self.audit(actor, "integration_role_deleted", role_id, {})
        await self._store.async_save(self._data)

    def ha_access_record(self, user_id: str) -> dict[str, Any]:
        """Return integration-owned metadata about native HA access for one user."""
        return deepcopy(self._data.get("ha_access", {}).get(user_id) or {})

    async def async_update_ha_access_record(
        self,
        actor,
        user_id: str,
        ...
        *,
        ...
        """Persist native HA permission backup/rules without touching HA auth storage."""
        current = self.ha_access_record(user_id)
        current.update(deepcopy(updates))
        current["updated_at"] = _utcnow()
        self._data.setdefault("ha_access", {})[user_id] = current
        self.audit(actor, action, user_id, deepcopy(updates))
        await self._store.async_save(self._data)
        return deepcopy(current)

    async def async_remove_ha_access_record(
        self, actor, user_id: str, *, action: str = "ha_access_record_removed"
    ) -> None:
        self._data.setdefault("ha_access", {}).pop(user_id, None)
        self.audit(actor, action, user_id, {})
        await self._store.async_save(self._data)

    def audit(
        self, actor, action: str, target: str, details: dict[str, Any]
    ) -> None:
        """Append a sanitized audit event. Secrets must never be passed in details."""
        ...
        def wrapper(hass, connection, msg, *args, **kwargs):
            ...
            if manager is None or not manager.can(user, permission):
                connection.send_error(
                    msg["id"], "unauthorized", f"Missing permission: {permission}"
                )
                return None
