...
class TemplateManagedEntity:
    ...
    async def async_added_to_hass(self) -> None:
        ...
        self.async_on_remove(
            async_dispatcher_connect(self.hass, SIGNAL_TEMPLATE_CHANGED, self._template_changed)
        )
    ...
    def _rewire_source_listener(self) -> None:
        ...
        if source:
            self._source_unsub = async_track_state_change_event(
                self.hass, [source], self._source_changed
            )
    ...
    def _template_changed(self) -> None:
        ...
        if current is None:
            self.hass.async_create_task(
                self.async_remove(), f"Remove template entity {self.entity_id}"
            )
            return
        self.record = current
