"""Light entities managed by Eshtaya Smart Control Template Manager."""
...
async def async_setup_entry(
    ...
    def add_entities() -> None:
        records = [
            item
            for item in manager.store.templates()
            if item.get("type") == "light" and not item.get("deferred")
        ]
        ...
        for record in records:
            ...
            if entity_id in known:
                continue
            known.add(entity_id)
            entities.append(ManagedTemplateLight(hass, manager, record))
        if entities:
...
class ManagedTemplateLight(TemplateManagedEntity, LightEntity):
    """Permanent light wrapper whose source is normally a Tuya switch."""
