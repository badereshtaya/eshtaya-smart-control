...
class TuyaManager:
    ...
    def _reload_config(self) -> None:
        ...
        self.client = TuyaOpenApiClient(
            self.hass, endpoint=self.endpoint, client_id=self.client_id, client_secret=self.client_secret
        )
        self._devices_at = 0.0
    ...
    async def async_test_config(self, config: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        return {"ok": True, "devices": len(devices), "endpoint": cfg[CONF_TUYA_ENDPOINT]}
    ...
    def _effective_candidate(self, raw: dict[str, Any]) -> dict[str, str]:
        ...
        endpoint = str(raw.get(CONF_TUYA_ENDPOINT, current.get(CONF_TUYA_ENDPOINT, ""))).strip().rstrip("/")
        if region != "custom":
            endpoint = TUYA_REGION_ENDPOINTS.get(region, endpoint)
        secret = str(raw.get(CONF_TUYA_CLIENT_SECRET, "")).strip() or str(current.get(CONF_TUYA_CLIENT_SECRET, "")).strip()
        client_id = str(raw.get(CONF_TUYA_CLIENT_ID, "")).strip() or str(current.get(CONF_TUYA_CLIENT_ID, "")).strip()
        uid = str(raw.get(CONF_TUYA_UID, "")).strip() or str(current.get(CONF_TUYA_UID, "")).strip()
        cfg = {
        ...
        if not all((cfg[CONF_TUYA_ENDPOINT], cfg[CONF_TUYA_CLIENT_ID], cfg[CONF_TUYA_CLIENT_SECRET], cfg[CONF_TUYA_UID])):
    ...
    async def async_list_devices(self, *, force: bool = False) -> list[dict[str, Any]]:
        ...
        if not force and self._devices and (time.monotonic() - self._devices_at) < self.CACHE_TTL:
            return list(self._devices)
        result = await self.client.async_request("GET", f"/v1.0/users/{self.uid}/devices")
    ...
    async def async_shadow_props(self, device_id: str) -> list[dict[str, Any]]:
        result = await self.client.async_request("GET", f"/v2.0/cloud/thing/{device_id}/shadow/properties")
    ...
    async def async_update_device_name(self, device_id: str, name: str) -> None:
        ...
        await self.client.async_request("PUT", f"/v1.0/devices/{device_id}", {"name": name})
        self._devices_at = 0.0

    async def async_update_prop_name(self, device_id: str, code: str, custom_name: str) -> None:
        ...
        async def one(device_id: str) -> dict[str, Any]:
            async with self._sem:
                try:
                    ...
                    switches = [
                        p for p in props if isinstance(p, dict) and (
                            str(p.get("code", "")).startswith("switch_")
                            or str(p.get("code", "")).startswith("socket_")
                            or str(p.get("code", "")) == "control"
                        )
                    ]
                    return {"device_id": device_id, "properties": switches, "ok": True}
                except Exception as err:
                    return {"device_id": device_id, "properties": [], "ok": False, "error": str(err)}
        ...
        for change in changes:
            try:
                ...
                if "name" in change and str(change.get("name") or "").strip():
                    await self.async_update_device_name(device_id, str(change["name"]))
                for prop in change.get("properties") or []:
                    if str(prop.get("custom_name") or "").strip():
                        await self.async_update_prop_name(device_id, str(prop.get("code") or ""), str(prop.get("custom_name") or ""))
                ok += 1
            except Exception:
                failed += 1
